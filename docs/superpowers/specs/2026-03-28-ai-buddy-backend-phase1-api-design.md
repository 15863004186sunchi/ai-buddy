...
Exchanges a valid refresh token for a new access token, and optionally a rotated refresh token.
...
- if connection is rejected or closed due to auth failure, front-end refreshes token first
- front-end reconnects with a fresh access token
